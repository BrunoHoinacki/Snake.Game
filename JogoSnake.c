...
void irparaxy(int x,int y)
{
    ...
    // x=j (sao colunas) e y=i (sao as linhas)
}
...
void bem_vindo()
{
    ...
    printf("MENU");
    irparaxy(15,12);
    printf("Selecione uma das opcoes", 135, 228);
    irparaxy(15,14);
    printf("<ENTER> Iniciar\t<ESC> Instrucoes\t<ESPACO> Dificuldade ", 135, 228);
...
void desenvolvimento()
{
    ...
    //d= direcao/ x= auxiliar/ t= auxiliar/ level= nivel k= controle do nivel/ 
    //pontos= pontuacao/ veloc= controle sleep/ resp= resposta para continuar
    ...
    do
    {
        // selecao para recomecar o jogo
        printf("\n\n\t\t Deseja come%car denovo? \n\t\t <1>SIM <2>N%cO \n", 135, 199);      
        scanf("%d", &resp);
    }
    while (resp!=1 && resp!=2);
    system("cls");
    if (resp==1)        // se 1 pressinado, ele retorna ao main e comeca tudo de novo
    {
        main();
    }
    else
